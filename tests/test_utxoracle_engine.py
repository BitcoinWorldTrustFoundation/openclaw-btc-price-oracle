...
from skills.precop_btc_price_announcer.utxoracle_engine import (
    UTXOracleEngine,
    InsufficientEntropyError
)
# Note: UTXOracleNodeError was removed as it's not defined in the current engine
from skills.precop_btc_price_announcer.utxoracle import UTXOracleClient, UTXOracleError
...
async def test_get_price_insufficient_entropy_abort(engine):
    ...
    assert "Impossible d'atteindre l'entropie requise" in str(exc_info.value)
