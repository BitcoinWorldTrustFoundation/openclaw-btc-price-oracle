#!/usr/bin/env python3
"""
precop_core.utxoracle — UTXOracle v9.1 RPC Adapter for PRECOP/BTCDAI.
...
PRECOP invariants (CLAUDE.md):
  ...
  - Caller is responsible for feeding the result into covenant collateral
    checks via `precop_core.collateral`, which uses only integer arithmetic.
...
Algorithm (12-step UTXOracle):
  ...
  Step 12    : (omitted — HTML plot not relevant for PRECOP).

References:
  - UTXOracle.py v9.1  https://utxo.live/oracle/
  - PRECOP Yellowpaper §6 (BTCDAI collateralization)
"""
...
def estimate_btc_price_cents(
    ...
    *,
    ...
    PRECOP invariant: this function does NOT use floating-point in the
